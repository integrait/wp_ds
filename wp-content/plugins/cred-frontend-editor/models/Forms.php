 ...
 */     
    ...
    public function getDefaultMessages()
    {
        ...
        if (!$messages)
        {
            $messages=array(
                ...
                'cred_message_post_saved'=>'Post Saved',
                //'cred_message_post_not_saved'=>'Post Not Saved',
                'cred_message_post_not_saved_singular'=>__('The post was not saved because of the following problem:<br />%PROBLEMS_UL_LIST','wp-cred'),
                'cred_message_post_not_saved_plural'=>__('The post was not saved because of the following %NN problems:<br />%PROBLEMS_UL_LIST','wp-cred'),
    ...
    public function changeFormat($fields)
    {
        ...
        if (isset($fields['notification']))
        {
            ...
            $notts->notifications=isset($nt->notifications)
                    ?$nt->notifications
                    :array();
...
//=================== GENERAL (CUSTOM) POST HANDLING METHODS ====================================================
    ...
    private function delete_post_taxonomies( $post )
    {
        ...
        foreach( $taxonomies as $taxonomy )
        {
            ...
            if (!isset($_POST[$todelete])) continue;
            //########################################################
    ...
    public function updatePost($post,$fields,$taxonomies=null)
    {
        ...
        if ($taxonomies)
        {
            $this->delete_post_taxonomies( $post );
            ...
            foreach ($taxonomies['flat'] as $tax)
            {
                ...
                $new_terms=(!empty($old_terms)) ? array_diff($old_terms,$tax['remove']) : array();
                // add new terms
