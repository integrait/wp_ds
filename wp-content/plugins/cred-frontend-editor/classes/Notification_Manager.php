...
final class CRED_Notification_Manager
{
    ...
    public static function evaluateConditions($notification, $fields, $snapshot)
    {
        ...
        foreach ($notification['event']['condition'] as $jj=>$condition)
        {
            ...
            //cred_log(array($snapshotFieldsHash, $fieldsHash));
            //cred_log($result);
            if ($condition['only_if_changed'] && isset($snapshotFieldsHash[$field]))
        ...
        foreach ($notification->notifications as $ii=>$notif)
        {
            ...
            if (isset($notif['event']))
            {
                ...
                if (isset($notif['event']['condition']) && !empty($notif['event']['condition']))
                {
                    ...
                    $conditionFields=$model->getPostFields( $post_id, $conditionFields );
                }
    ...
    {
        if (empty($hash) || ''==$hash)  return array();
    ...
    {
        // bypass if nothing
        if (!$notification || empty($notification) /*|| !isset($notification['to']['type'])*/)  return array('error'=>__('No Notification given','wp-cred'));
        ...
        if (!$recipients || empty($recipients)) return array('error'=>__('No recipients specified', 'wp-cred'));
    ...
    public static function sendNotifications($post_id, $form_id, $notificationsToSent)
    {
        ...
        foreach ($notificationsToSent as $notification)
        {
            ...
            if (
                !$notification
                || empty($notification)
                || !(isset($notification['to']['type']) || isset($notification['to']['author']))
            ) {
                continue;
            }
            ...
            if (in_array('user_id_field', $notification['to']['type']))
            {
                ...
                $user_id=@trim($model->getPostMeta($post_id, $notification['to']['user_id_field']['field_name']));
            ...
            $recipients=apply_filters('cred_notification_recipients', $recipients, $notification, $form_id, $post_id);
            if (!$recipients || empty($recipients)) continue;
