...
/**
 *
 * $HeadURL: https://www.onthegosystems.com/misc_svn/crud/tags/1.3.3/classes/Form_Builder.php $
 * $LastChangedDate: 2014-09-16 11:14:40 +0000 (Tue, 16 Sep 2014) $
...
class CRED_Form_Builder implements CRED_Friendable, CRED_FriendableStatic
{
    ...
    */
    ...
    public static function has_form() { return (StaticClass::$_staticGlobal['COUNT']>0); }
    ...
    private static function getCachedForm($form_id, $post_id, $preview, $force_count=false, $specific_post_id=null)
    {
        ...
        )
        {
            ...
            $form=$fb->form($form_id, $post_id, $preview, $form_count, $specific_post_id);
    ...
    public function hasHideComments() { $fields=$this->_formData->getFields(); return $fields['form_settings']->form['hide_comments'];}
    // get extra javascript/css attached to this form
    public function getExtra() { $fields=$this->_formData->getFields();  return $fields['extra'];}
    ...
    public function form($form_id, $post_id=null, $preview=false, $force_form_count=false, $specific_post_id=null)
    {
        ...
        $this->_formData=new FormData($form_id, $preview);
        $form=&$this->_formData;
            ...
            if (!$zebraForm->preview)
            {
                ...
                if (is_wp_error($post_id)) {
                    ...
                    if (is_int($post_id) && $post_id>0)
                    {
                        ...
                        $zebraForm->add_field_message($formHelper->getLocalisedMessage('post_saved'));
                   }
                   else
                    {
...
//        }
    ...
    private function parseInputs($form_id, $post_id=null, $preview=false, $force_form_count=false, $specific_post_id=null)
    {
        ...
        if ($formHelper->isError($form)) return $form;
        ...
        {    
            ...
            if ($formHelper->isError($this->_postData)) return $this->_postData;
        ...
        $this->_zebraForm = new CredForm($my_form_id, $post_id, $actionUri, $preview);
    ...
    private function CRED_build() {
        ...
        if ($zebraForm->preview) $preview_content=$this->_content;
        ...
        if ($zebraForm->preview)
        {
            ...
            $zebraForm->add_field_message(__('Form Preview Mode','wp-cred'));
        }
        // hidden fields are rendered automatically
        // add form id
    ...
    private function validate()
    {
        ...
        if ($zebraForm->isSubmitted())
        {
                    ...
                    ))
            {
                ...
                $rcfield = (!empty($_POST["recaptcha_challenge_field"])) ? $_POST["recaptcha_challenge_field"] : "";
                $rrfield = (!empty($_POST["recaptcha_response_field"])) ? $_POST["recaptcha_response_field"] : "";
            ...
            return $zebraForm->validate( $this->_post_ID, /*$formHelper->get_form_field_values()*/ $zebraForm->form_properties['fields'] );
        }
        return $result;
    }
...
//        }
        ...
        if ($all_ok)
        {
            ...
            if (!isset($post->post_type)||empty($post->post_type)) $post->post_type=$post_type;
    ...
    private function save($post_id=null)
    {
        ...
        // do custom actions before post save
        do_action('cred_before_save_data_form_'.$form_slug, $thisform);
        do_action('cred_before_save_data_'.$form_id, $thisform);
        do_action('cred_before_save_data', $thisform);
