...
class CredFormCreator {
    ...
    public static function taxOutput($tax, $form_id, $form_name, $WPML, $pad) {
        ...
        if ($WPML)
            $tax_out[] = $pad . PAD . '<div class="cred-label"><h2>[wpml-string context="cred-form-' . $form_name . '-' . $form_id . '" name="' . $tax->label . '"]' . $tax->label . '[/wpml-string]</h2></div>';
        else
            $tax_out[] = $pad . PAD . '<div class="cred-label"><h2>' . $tax->label . '</h2></div>';
    ...
    public static function groupOutput($group, $fields, $conditions, $custom_fields, $form_id, $form_name, $WPML, $pad) {
        ...
        $group_out[] = $pad . '<div class="cred-group ' . $group_class_slug . '">';
        if ($WPML) {
            $group_out[] = $pad . PAD . '<div><h2>[wpml-string context="cred-form-' . $form_name . '-' . $form_id . '" name="' . $group . '"]' . $group . '[/wpml-string]</h2></div>';
        } else {
            $group_out[] = $pad . PAD . '<div><h2>' . $group . '</h2></div>';
        }
    ...
    public static function fieldOutput($field, $form_id, $form_name, $WPML, $pad, $extra = array()) {
        ...
        if (isset($field)) {
            ...
            if ('checkbox' != $field->type) {
                $field_out[] = $pad . PAD . '<div class="cred-label">';
                if ($WPML) {
