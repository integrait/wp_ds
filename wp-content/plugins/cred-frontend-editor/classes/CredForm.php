...
/**
 *
 * $HeadURL: https://www.onthegosystems.com/misc_svn/crud/tags/1.3.3/classes/CredForm.php $
 * $LastChangedDate: 2014-09-19 08:34:48 +0000 (Fri, 19 Sep 2014) $
 * $LastChangedRevision: 27303 $
...
class CredForm {
    ...
    public function __construct($form_id, $current_postid, $actionUri, $preview = false) {
    ...
    public function cred_field_shortcodes($atts) {
        ...
        if ($field == 'form_messages') {
            return '<label id="wpt-form-message" 
                data-message-single="' . esc_js($formHelper->getLocalisedMessage('post_not_saved_singular')) . '"
                data-message-plural="' . esc_js($formHelper->getLocalisedMessage('post_not_saved_plural')) . '" 
                style="display:none;" class="wpt-form-error">test</label><!CRED_ERROR_MESSAGE!>';
        }
        // make boolean
        ...
        if (!$taxonomy) {
            ...
            if (in_array($taxonomy, array_keys($this->out_['fields']['taxonomies'])) &&
                ...
                if (// auxilliary field type matches taxonomy type
                    ...
                    return '<div class="js-taxonomy-button-placeholder" data-taxonomy="' . $taxonomy . '" style="display:none"></div>';
                    /*
                      $field=array(
