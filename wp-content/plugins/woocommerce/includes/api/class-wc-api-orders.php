...
class WC_API_Orders extends WC_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		# Get all distributor's orders
		# GET|PUT /orders/<id>/vendor/
		$routes[ $this->base . '/(?P<id>\d+)/vendor' ] = array(
			array( array( $this, 'get_distributor_orders' ), WC_API_Server::READABLE ),
			array( array( $this, 'edit_distributor_orders' ), WC_API_SERVER::EDITABLE | WC_API_Server::ACCEPT_DATA ),
		);
	...
	public function get_order( $id, $fields = null, $filter = array() ) {
		...
		$order_data = array(
			...
			'fee_lines'                 => array(),
			'coupon_lines'              => array(),
			'distributor'	            => get_post_meta($order->id, 'distributor_order', true)
		...
		foreach( $order->get_items() as $item_id => $item ) {
			...
			$order_data['line_items'][] = array(
				...
				'meta'         => $item_meta,
				'line_note'    => $this->ds_get_line_note($order->id, ( isset( $product->variation_id ) ) ? $product->variation_id : $product->id),
				'in_stock'     => $this->ds_item_in_stock($order->id, ( isset( $product->variation_id ) ) ? $product->variation_id : $product->id)
			);
		}
	 ...
	 * @return string note
	 */
	function ds_get_line_note($order_id , $item_id){
		global $wpdb;

		$line_notes = $wpdb->get_results("SELECT notes FROM {$wpdb->prefix}routed_order_items WHERE item_id = $item_id AND order_id = $order_id group by order_id ORDER BY created_at DESC");
		$line_note = ( count($line_notes) > 0 )? $line_notes[0]->notes : "";

		return $line_note;
	}

	/**
	 * Check if line item in stock
	 * 
	 * @param int $order_id Order ID
	 * @param int $item_id Order line item ID 
	 * @return string note
	 */
	function ds_item_in_stock($order_id , $item_id){
		global $wpdb;

		$line_stock = $wpdb->get_results("SELECT in_stock FROM {$wpdb->prefix}routed_order_items WHERE item_id = $item_id AND order_id = $order_id group by order_id ORDER BY created_at DESC");
		$in_stock = ( count($line_stock) > 0 )? $line_stock[0]->in_stock : "0";
	 ...
	 * @return array
	 */
	public function get_distributor_orders($id, $fields = null, $filter = array(), $page = 1){

		global $wpdb;

		$user = get_userdata($id);

		$routed_orders = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}routed_order_items WHERE distributor = '$id' group by order_id ORDER BY created_at DESC");
		$total_orders = 0;

		$orders = array();

		foreach ($routed_orders as $_order) {
			
			if ( ! $this->is_readable( $_order->order_id ) )
				continue;

			$order = new WC_Order($_order->order_id);  

			if($_order->status != 'trash'){ 
				$orders[] = current( $this->get_order( $_order->order_id, null ) );
			}
		}  
	...
	private function set_line_item( $order, $item, $action ) {
		...
		/** DrugStoc Custom Meta */
		// supplier meta 
		if ( isset( $item['line_note'] ) || isset( $item['in_stock'] ) ) {
			$item_args['line_note'] = $item['line_note'];
			$item_args['in_stock'] = $item['in_stock'];
			...
			if($creating){
				...
				$wpdb->prepare( 
					...
					WHERE order_id = %d AND item_id = %d",
				        (int)$item['in_stock'], 
				        strip_tags( stripslashes($item['line_note'])), 
				        $order->id, 
				        $item['product_id'])
			    );
			} 
