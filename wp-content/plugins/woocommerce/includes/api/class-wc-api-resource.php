...
class WC_API_Resource {
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// Throttling - Log API Request per user
		global $wpdb;
		...
		// Check if Resouce already exists for user  
		$requests = $wpdb->get_results("SELECT user_id, resource, requests_throttled FROM {$wpdb->prefix}ds_api_throttle_log WHERE user_id = {$user->ID} and resource = '{$resource_name}'");
		$request = $requests->resource; 

	    	// Update request count if resource already exists 
	    	if(count($requests) > 0){
		    // Update request count
		    $wpdb->query( $wpdb->prepare( 
				"UPDATE {$wpdb->prefix}ds_api_throttle_log
				SET resource = %s, 
				requests_throttled = requests_throttled + 1,
				last_throttled = %s 
				WHERE user_id = %d", 
		        array( $resource_name, $date, $user->ID ) 
			));
	    	}else{
			// Insert new resource request
		    $wpdb->insert(
				"{$wpdb->prefix}ds_api_throttle_log", 
				array( 
					'user_id'		=> $user->ID,
