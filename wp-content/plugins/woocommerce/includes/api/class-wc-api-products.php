...
class WC_API_Products extends WC_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		# Create Price Model 
		# POST /products/price/<id>
		$routes[ $this->base . '/price/(?P<id>\d+)' ] = array(
			array( array( $this, 'add_product_price_meta' ), WC_API_Server::CREATABLE | WC_API_Server::ACCEPT_DATA),
		);

		# Get Distributor Products  
		# GET /products/<id>/vendor
		$routes[ $this->base . '/(?P<id>\d+)/vendor' ] = array(
			array( array( $this, 'get_distributor_products' ), WC_API_Server::READABLE ), 
		);		
	...
	private function get_product_data( $product ) {
		...
		return array(
			...
			'variations'         => array(),
			'parent'             => array(),
			'price_meta' 	     => $this->get_pricemeta($product->id)  
		);
	}

	/** 
	 * Get all _price related post meta for a product
	 *
	 * @param int $id Product ID
	 * @return Array Returns an array of all distrbutor prices associated with the product
	 */
	function get_pricemeta($id)
	{
		global $wpdb;

		$products = array();
		
		$all = $wpdb->get_results("SELECT w.meta_key, w.meta_value FROM wp_postmeta w INNER JOIN wp_posts p ON w.post_id = p.ID WHERE p.ID = $id AND w.meta_key NOT IN ('_regular_price','_sale_price','_price') AND w.meta_key LIKE '%_price'");
		
		foreach ($all as $key => $product) $products[$product->meta_key] = $product->meta_value;
	...
	/** 
	 * Attach new Product Price from DIMS
	 * 
	 * @since 2.2
	 * @param int $id
	 * @param array $data
	 * @return bool|WP_Error
	 */
	public function add_product_price_meta( $id, $data ){
		...
		$product = wc_get_product( $id );
		$meta = ( isset($data['price_meta_key']) )? $data['price_meta_key'] : "";

		// add data that applies to this product type 
		if ( isset( $data['price_meta_value']) && $meta != "") {
			update_post_meta( $id, $meta, floatval($data['price_meta_value']));
 		} else {
			return new WP_Error( "woocommerce_api_invalid_price", sprintf( __( 'Invalid price meta for %s', 'woocommerce' ), $product->get_title() ), array( 'status' => 404 ) );
		} 
	 ...
	 * @return bool|WP_Error
	 */
	public function get_distributor_products( $id, $fields = null, $filter = array()){  
		// 	$fields = null, $type = null, $filter = array(), $page = 1 

		global $wpdb; 
		
		$user = get_userdata($id);

		$distrbutor = get_user_meta($id, 'primary_distributor', true);

    	// Get product ids 
    	$product_ids = $wpdb->get_results("SELECT p.ID FROM {$wpdb->prefix}posts p INNER JOIN {$wpdb->prefix}postmeta w ON p.ID = w.post_id WHERE p.post_type='product' AND p.post_status='publish' AND w.meta_key='{$distrbutor}' AND w.meta_value != ''");
    
    	$products = array();  

		foreach ( $product_ids as $key => $product_id ) {

			if ( ! $this->is_readable( $product_id->ID ) ) {
				continue;
			}		     
 
			 $products[] = array( 'product_id' => $product_id->ID);
			//$products[] = current( $this->get_product( $product_id->ID, null ) );  
		}  
