...
if(!class_exists('RedistributorOrders')){ 
	...
	class RedistributorOrders  
	{
		...
		function redistributor_order_list(){
			...
			<table id="ds_redistributor" class="wp-list-table widefat fixed posts">
			<thead>
				<tr>
			...
			foreach ($routed_orders as $customer_order) {
				...
				?>
					<tr>
						...
						<td><?php echo date("d M Y H:m", strtotime($order->order_date));?></td>
							...
							<tr>
								...
								<td>
									...
									<?php if($order->status == 'on-hold'){?> 
										<a id="m_process" title="Mark Processing" class="button tips processing" href="#">Mark Processing</a>
									<?php }else if($order->status == 'processing' || $order->status == 'on-hold'){?>	
										<a id="m_complete" title="Mark Complete" class="button tips complete" href="#">Mark Complete</a>
									<?php }?>
									</p>
									<p class="form-field form-field-wide">
