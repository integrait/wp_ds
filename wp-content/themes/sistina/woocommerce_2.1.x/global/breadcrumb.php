...
if( !is_woocommerce() ||
    is_shop() && ! yit_get_option('shop-show-breadcrumb') ||
    ( is_shop() || is_product_category() || is_product_tag() ) && ! yit_get_option('shop-single-show-breadcrumb')
) return;
...
$shop_page_id = yith_wc_get_page_id( 'shop' );
$shop_page    = get_post( $shop_page_id );
$current_pattern = '<span class="current">%s</span>';

// If permalinks contain the shop page in the URI prepend the breadcrumb with shop
if ( $shop_page_id && is_object( $shop_page ) && strstr( $permalinks['product_base'], '/' . $shop_page->post_name ) && get_option( 'page_on_front' ) !== $shop_page_id ) {
	$prepend = $before . '<a href="' . get_permalink( $shop_page ) . '">' . $shop_page->post_title . '</a> ' . $after . $delimiter;
}

if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == yith_wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		$_name = yith_wc_get_page_id( 'shop' ) ? get_the_title( yith_wc_get_page_id( 'shop' ) ) : '';
