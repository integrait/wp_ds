...
if( !function_exists( 'yit_og_meta_tags' ) ) {
    ...
    function yit_og_meta_tags( $post_id ) {
        ...
        if( !yit_get_option('enable-open-graph') ) return;
        ...
        if( isset( $queried_object->post_type ) ) {
            ...
            $ogcontent = $post->post_excerpt ? $post->post_excerpt : wp_trim_words( $post->post_content );
        ...
        ?><meta property="og:description" content="<?php echo trim( str_replace( array( "\n", "\t" ), '', strip_tags( strip_shortcodes( $ogcontent ) ) ) ) ?>" /><?php
    }
}
    ...
    function yit_breadcrumb( $delimiter = '&raquo;' ) {
        ...
        if ( is_single() && yit_get_model('cpt_unlimited')->is_cptu( $post->post_type ) ) {
            ...
            if ( get_post_type() != 'post' ) {
                ...
                if ( isset( $get_the_title ) && !empty($get_the_title) && $get_the_title !='' ){
                    echo $before . get_the_title() . $after;
                }
            }
    ...
    function yit_pagination( $pages = '', $range = 10 ) {
        ...
        if ( $paged === false ) $paged = 1;
         ...
         if( 1 != $pages ) {
             ...
             if( $paged > 2 ) $html .= sprintf('<a class="%s" href="%s">&laquo;</a>', 'yit_pagination_first', get_pagenum_link( 1 ));
             if( $paged > 1 ) $html .= sprintf('<a class="%s" href="%s">&lsaquo;</a>', 'yit_pagination_previous', get_pagenum_link( $paged - 1 ));
             ...
             if ( $paged < $pages )     $html .= sprintf('<a class="%s" href="%s">&rsaquo;</a>', 'yit_pagination_next', get_pagenum_link( $paged + 1 ));
             if ( $paged < $pages - 1 ) $html .= sprintf('<a class="%s" href="%s">&raquo;</a>', 'yit_pagination_last', get_pagenum_link($pages));
