...
class YIT_Walker_Nav_Menu extends Walker_Nav_Menu
{
	...
	public function getOutput( $item, $fieldId, $field ) {
		if( $field['type'] == 'text' ) {
			...
			$img_url = $item->{$fieldId};
			$img_array = yit_getimagesize( $img_url );
			
			return "<a class='custom-item-{$item->ID} custom-item-{$fieldId} custom-item-image' href='". esc_attr( $item->url ) ."'><img src='". $img_url ."' alt='". apply_filters( 'the_title', $item->title, $item->ID ) ."' " . $img_array[3] . " /></a>";
		}
	}
}
