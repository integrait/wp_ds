...
if ( $available_methods ) {
        ...
        if ( $method->cost > 0 ) {
            if ( WC()->cart->tax_display_cart == 'excl' ) {
                ...
                if ( $method->get_shipping_tax() > 0 && WC()->cart->prices_include_tax ) {
                    $method->full_label .= ' <small>' . WC()->countries->ex_tax_or_vat() . '</small>';
                }
            } else {
                $method->full_label .= ': ' . wc_price( $method->cost + $method->get_shipping_tax() );
                if ( $method->get_shipping_tax() > 0 && ! WC()->cart->prices_include_tax ) {
                    $method->full_label .= ' <small>' . WC()->countries->inc_tax_or_vat() . '</small>';
                }
            }
        } elseif ( $method->id !== 'free_shipping' ) {
            $method->full_label .= ' (' . __( 'Free', 'yit' ) . ')';
        }
    ...
    if ( 1 === count( $available_methods ) ) {
        ...
        foreach ( $available_methods as $method )
            echo '<option value="' . esc_attr( $method->id ) . '" ' . selected( $method->id, WC()->session->chosen_shipping_method, false ) . '>' . wp_kses_post( $method->full_label ) . '</option>';
        ...
        foreach ( $available_methods as $method )
            echo '<li><input type="radio" name="shipping_method" id="shipping_method_' . sanitize_title( $method->id ) . '" value="' . esc_attr( $method->id ) . '" ' . checked( $method->id, WC()->session->chosen_shipping_method, false) . ' /> <label for="shipping_method_' . sanitize_title( $method->id ) . '">' . wp_kses_post( $method->full_label ) . '</label></li>';
...
// No shipping methods are available
    ...
    if ( ! WC()->customer->get_shipping_country() || ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) {
        ...
        echo '<p>' . __( 'Please fill in your details to see available shipping methods.', 'yit' ) . '</p>';

    } else {

        $customer_location = WC()->countries->countries[ WC()->customer->get_shipping_country() ];
