...
<tr class="shipping">
    <th colspan="2"><?php
        if ( $show_package_details ) {
            printf( __( 'Shipping #%d', 'yit' ), $index + 1 );
        } else {
            _e( 'Shipping', 'yit' );
        }
        ?></th>
    <td colspan="2">
        ...
        <?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
            ...
            <?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'yes' ) : ?>
                ...
                <p><?php _e( 'No shipping methods were found; please recalculate your shipping or continue to checkout and enter your full address to see if there is shipping available to your location.', 'yit' ); ?></p>
            ...
            <?php if ( is_cart() ) : ?>
                ...
                <?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
                    '<div class="woocommerce-error"><p>' .
                        sprintf( __( 'Sorry, shipping is unavailable %s.', 'yit' ) . ' ' . __( 'If you require assistance or wish to make alternate arrangements please contact us.', 'yit' ), WC()->countries->shipping_to_prefix() . ' ' . WC()->countries->countries[ WC()->customer->get_shipping_country() ] ) .
                        '</p></div>'
                ); ?>
