...
$user_img_url = get_user_meta($user->ID, 'cupp_upload_meta', true);
...
$j_address = $address;//get_user_meta($user->ID, 'billing_address_1', true);// . "  " . get_user_meta($user->ID, 'billing_city', true);
...
<div id="dist_category" style="display:none">
	...
	<select onChange="window.location.href=this.value" class="distComposition">
		...
		foreach ($pdt_comp as $key => $composition) { ?>
		    <option value='<?php echo home_url("/vendor/$distributor/?category=$dist_category&composition={$composition->slug}&manufacturer=$pa_manufacturer$search_param")?>' <?php echo ($pa_composition == $composition->slug)? 'selected':'' ?> > <?php echo "{$composition->name} ({$composition->no_of_pdts})"; ?> </option>
		<?php
		}?>
	</select><br/>
	...
	<select onChange="window.location.href=this.value" class="distManufacturer">
		...
		foreach ($pdt_manuf as $key => $manufacturer) { ?>
		    <option value='<?php echo home_url("/vendor/$distributor/?category=$dist_category&composition=$pa_composition&manufacturer={$manufacturer->slug}$search_param")?>' <?php echo ($pa_manufacturer == $manufacturer->slug)? 'selected':'' ?> > <?php echo "{$manufacturer->name} ({$manufacturer->no_of_pdts})"; ?> </option>
		<?php
		}?>
	</select>
    ...
    <div class="map-user-profile row">
        <div class="user-info-cn span3">
          ...
          <div class="co-info-holder">
            ...
            <p class="user-status-badge">  <img src="<?php echo home_url('/'); ?>wp-content/themes/sistina/images/dist-icon.png"><span>Verified Distributor</span></p><br>
        ...
        <div class="map-info-cn span9">
        	...
        	<div id="tab-map" class="tab-pane active">
	            <?php
	            	...
	            	echo "<script> draw_single_map('$institution', '$j_address','$j_state','$j_email' ,'$gmap_lat', '$gmap_lon', 'Distributor'); </script>";
	            ?>
        	</div>
        </div>
    </div>
