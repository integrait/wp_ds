...
// $distributor = $wp_query->query_vars['m'];
...
// $page_num = array_search('page', $paged) !== false? $paged[ array_search('page', $paged) + 1 ]:1;
...
// $_GLOBAL['wp_query'] = new WP_Query($args);
query_posts( $args );
// query_posts();
// Filtered IDs
...
$pdt = $wpdb->get_results("SELECT t.name, t.slug, count(t.term_id) as no_of_pdts FROM wp_terms as t
  ...
  WHERE p.taxonomy LIKE 'product_cat' and wtr.object_id IN ($post_ids) GROUP BY t.name ORDER BY no_of_pdts DESC ");
...
<div id="dist_category" style="display:none">  
	...
	<ul class="distCategory">
		...
		foreach ($pdt as $key => $category) { ?>
		    <li><a href='<?php echo home_url("/m/$manu_slug/?category={$category->slug}&composition=$pa_composition$search_param")?>' style="cursor: pointer;"> <?php echo "{$category->name} ({$category->no_of_pdts})"; ?> </a></li>
		<?php
		}?>
	</ul>
	<br/>
		...
		$manufacturers_dist = $wpdb->get_results("SELECT w.* FROM `wp_users` as w INNER JOIN wp_usermeta as m on w.ID = m.user_id WHERE m.meta_key = 'primary_distributor' and m.meta_value!=''");
      	foreach ($manufacturers_dist as $key => $_user) {
	        ...
	        if(in_array('shop_manager', $user->roles ) ) {
      		...
      		<li>
      			<a href="<?php echo home_url('/vendor/'.substr($dist, count($dist)-1, -6));?>" style="color:black">
...
<div style="padding-right: 30px">
	<h3 style="color: #ff9700;font-size: 20px;margin-left: 42px;font-weight: 400;">Distributors</h3> 
		<div style="float: left; margin-bottom: 30px;" class="responsive" id="show_all_dist">
			<ul style="list-style:none; float: left; margin-bottom: 30px;" class="responsive" id="show_all_dist">  
	    <?php
			$manufacturers_dist = $wpdb->get_results("SELECT w.* FROM `wp_users` as w INNER JOIN wp_usermeta as m on w.ID = m.user_id WHERE m.meta_key = 'primary_distributor' and m.meta_value!=''");
	      	foreach ($manufacturers_dist as $key => $_user) {
	      		$user = get_user_by('id', $_user->ID);
