...
if( $services->have_posts() ) :
	...
	?>
	<div class="section services margin-top section-services-bandw">
		<?php if( !empty( $title ) ) {
...
<script>
jQuery(function($){
    $(document).on('hover', '.service-wrapper', function(e){
        var services = $('.service-wrapper');
        if(e.type == 'mouseenter') {
            services.not(this).stop(true, false).animate({opacity:0.6}, 300);

        } else if ( e.type == 'mouseleave' ) {
            services.not(this).delay(10).animate({opacity:1}, 300);
        }
    });
})
</script>
