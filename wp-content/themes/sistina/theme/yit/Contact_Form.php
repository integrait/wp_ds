    ...
    protected function _sendEmail()
    {
    	...
    	if ( isset( $_POST['yit_action'] ) AND ( $_POST['yit_action'] == 'sendemail' OR $_POST['yit_action'] == 'sendemail-ajax' ) )
    	{
            ...
            if( $this->get('enable_captcha') == '1' ){
                ...
                if( ! $resp->is_valid ) {
                    $this->messages[ $this->current_form ]['general'] = '<p class="error">' . $resp->error . '</p>';
                    return;
                }
            }
            ...
            $post_data = array_map( 'stripslashes_deep', (array) $_POST['yit_contact'] );
    		...
    		// if there were some errors, return messages.
            $form_messages = array();
            if( isset( $this->messages[ $this->current_form ] ) ){
                $form_messages = $this->messages[ $this->current_form ];
            }
            if( ! empty( $form_messages ) ) {
                if( sizeof( $form_messages ) >= 1 && reset( $form_messages ) ) {
                    return;
                }
            }
    	...
    	foreach( $fields as $id => $field )
    	{
			...
			*/
    		...
    		switch( $field['type'] )
    		{
    			...
    			// textarea
    			case 'textarea':
    				$html .= "<textarea name=\"yit_contact[" . $field['data_name'] . "]\" id=\"" . $field['data_name'] . "-$form_name\" rows=\"8\" cols=\"". apply_filters('yit_contact_form_cols', 30, $form_name) ."\" class=\"" . implode( $input_class, ' ' ) . "\" placeholder=\"" . stripslashes_deep( $field['title'], 'highlight-text' ) . "\">$value</textarea>";
    			break;
